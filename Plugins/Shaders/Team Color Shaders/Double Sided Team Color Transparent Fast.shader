Shader "Ciconia Studio/Effects/Team Color/2Sided/Transparent/Fast" {
    Properties {
        [Space(15)][Header(Main Maps)]
        [Space(10)]_Color ("Color", Color) = (1,1,1,1)
        _MainTex ("Albedo (TransA)", 2D) = "white" {}
        ...
        [Space(15)][Header(Transparent Properties)]
        [Space(10)][MaterialToggle] _DisableAlbedoAlphaChannel ("Disable Albedo Alpha Channel", Float ) = 1
        [MaterialToggle] _InvertAlbedoAlpha ("Invert Albedo Alpha", Float ) = 1
...
////// Specular:
                ...
                float3 Specularmap = (_SpecColor.rgb*saturate(max((Mask*_SpecularIntensityMask),(_SpecularIntensity*(1.0 - Mask)))));
...
/// Final Color:
                ...
                float ADiffuse = _MainTex_var.a;
                float node_1940 = ADiffuse;
                float Transparent = (lerp( lerp( (1.0 - node_1940), node_1940, _InvertAlbedoAlpha ), 1.0, _DisableAlbedoAlphaChannel )-lerp(-1,1,_Transparency));
                fixed4 finalRGBA = fixed4(finalColor,Transparent);
...
/// Final Color:
                ...
                float node_1940 = ADiffuse;
                float Transparent = (lerp( lerp( (1.0 - node_1940), node_1940, _InvertAlbedoAlpha ), 1.0, _DisableAlbedoAlphaChannel )-lerp(-1,1,_Transparency));
                fixed4 finalRGBA = fixed4(finalColor * Transparent,0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
