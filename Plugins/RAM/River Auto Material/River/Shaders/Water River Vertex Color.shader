Shader "NatureManufacture Shaders/Water/Water River Vertex Color"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float Globaltiling819 = ( 1.0 / _GlobalTiling );
			float2 temp_output_821_0 = ( uv_WaterNormal * Globaltiling819 );
			float2 panner612 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _WaterMixSpeed :  appendResult706 ) + temp_output_821_0);
			float2 WaterSpeedValueMix516 = panner612;
			float2 appendResult705 = (half2(_WaterMainSpeed.y , _WaterMainSpeed.x));
			float2 panner611 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _WaterMainSpeed :  appendResult705 ) + temp_output_821_0);
			...
			float2 temp_output_823_0 = ( uv_SmallCascadeNormal * Globaltiling819 );
			float2 panner597 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _SmallCascadeMixSpeed :  appendResult709 ) + temp_output_823_0);
			...
			float2 panner598 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _SmallCascadeMainSpeed :  appendResult710 ) + temp_output_823_0);
			...
			float2 temp_output_825_0 = ( uv_BigCascadeNormal * Globaltiling819 );
			float2 panner606 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _BigCascadeMixSpeed :  appendResult712 ) + temp_output_825_0);
			float2 BigCascadeSpeedValueMix608 = panner606;
			float2 appendResult714 = (half2(_BigCascadeMainSpeed.y , _BigCascadeMainSpeed.x));
			float2 panner607 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _BigCascadeMainSpeed :  appendResult714 ) + temp_output_825_0);
			...
			float lerpResult808 = lerp( pow( ( temp_output_89_0 + _ShalowDepth ) , _ShalowFalloff ) , 100.0 , ( _BigCascadeTransparency * clampResult514 ));
			float temp_output_94_0 = saturate( lerpResult808 );
			...
			float2 panner116 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _FoamSpeed :  appendResult716 ) + ( uv_Foam * Globaltiling819 ));
			...
			float2 temp_output_829_0 = ( uv_Noise * Globaltiling819 );
			float2 panner646 = ( _SinTime.x * ( temp_output_743_0 * float2( -1.2,-0.9 ) ) + temp_output_829_0);
			half4 tex2DNode647 = tex2D( _Noise, panner646 );
			float2 panner321 = ( _SinTime.x * temp_output_743_0 + temp_output_829_0);
			...
			float clampResult803 = clamp( ( pow( min( tex2DNode647.a , tex2D( _Noise, ( panner321 + float2( -0.47,0.37 ) ) ).a ) , _BigCascadeNoisePower ) * 20.0 ) , 0.0 , 1.0 );
			float lerpResult626 = lerp( ( tex2DNode213.a * 0.5 ) , tex2DNode213.a , clampResult803);
			...
			float lerpResult816 = lerp( _WaterSpecularFar , _WaterSpecularClose , pow( temp_output_94_0 , _WaterSpecularThreshold ));
			float lerpResult130 = lerp( lerpResult816 , _FoamSpecular , temp_output_114_0);
