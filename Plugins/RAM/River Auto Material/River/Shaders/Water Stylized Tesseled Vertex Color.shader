Shader "NatureManufacture Shaders/Water/Water Stylized Tesseled Vertex Color"
{
	Properties
	{
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v )
		{
			...
			float Globaltiling827 = ( 1.0 / _GlobalTiling );
			float2 temp_output_828_0 = ( uv_WaterNormal * Globaltiling827 );
			float2 panner612 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _WaterMixSpeed :  appendResult706 ) + temp_output_828_0);
			float2 WaterSpeedValueMix516 = panner612;
			float2 appendResult705 = (half2(_WaterMainSpeed.y , _WaterMainSpeed.x));
			float2 panner611 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _WaterMainSpeed :  appendResult705 ) + temp_output_828_0);
			float2 WaterSpeedValueMain614 = panner611;
			float temp_output_415_0 = ( ( _WaterTessScale * tex2Dlod( _WaterTesselation, half4( WaterSpeedValueMix516, 0, 1.0) ).a ) + ( _WaterTessScale * tex2Dlod( _WaterTesselation, half4( WaterSpeedValueMain614, 0, 1.0) ).a ) );
			...
			float2 temp_output_833_0 = ( uv_SmallCascadeNormal * Globaltiling827 );
			float2 panner597 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _SmallCascadeMixSpeed :  appendResult709 ) + temp_output_833_0);
			...
			float2 panner598 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _SmallCascadeMainSpeed :  appendResult710 ) + temp_output_833_0);
			float2 SmallCascadeSpeedValueMain600 = panner598;
			float temp_output_414_0 = ( ( tex2Dlod( _SmallCascadeWaterTess, half4( SmallCascadeSpeedValueMix433, 0, 1.0) ).a * _SmallCascadeWaterTessScale ) + ( tex2Dlod( _SmallCascadeWaterTess, half4( SmallCascadeSpeedValueMain600, 0, 0.0) ).a * _SmallCascadeWaterTessScale ) );
			...
			float2 temp_output_835_0 = ( uv_BigCascadeNormal * Globaltiling827 );
			float2 panner606 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _BigCascadeMixSpeed :  appendResult712 ) + temp_output_835_0);
			float2 BigCascadeSpeedValueMix608 = panner606;
			float2 appendResult714 = (half2(_BigCascadeMainSpeed.y , _BigCascadeMainSpeed.x));
			float2 panner607 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _BigCascadeMainSpeed :  appendResult714 ) + temp_output_835_0);
			float2 BigCascadeSpeedValueMain432 = panner607;
			float temp_output_565_0 = ( ( _BigCascadeWaterTessScale * tex2Dlod( _BigCascadeWaterTess, half4( BigCascadeSpeedValueMix608, 0, 1.0) ).a ) + ( _BigCascadeWaterTessScale * tex2Dlod( _BigCascadeWaterTess, half4( BigCascadeSpeedValueMain432, 0, 1.0) ).a ) );
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float2 panner116 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _FoamSpeed :  appendResult716 ) + ( uv_Foam * Globaltiling827 ));
			...
			float2 temp_output_839_0 = ( uv_Noise * Globaltiling827 );
			float2 panner646 = ( _SinTime.x * ( temp_output_743_0 * float2( -1.2,-0.9 ) ) + temp_output_839_0);
			half4 tex2DNode647 = tex2D( _Noise, panner646 );
			float2 panner321 = ( _SinTime.x * temp_output_743_0 + temp_output_839_0);
			float clampResult488 = clamp( ( pow( min( tex2DNode647.a , tex2D( _Noise, panner321 ).a ) , _SmallCascadeNoisePower ) * 20.0 ) , 0.0 , 1.0 );
			float lerpResult480 = lerp( 0.0 , tex2D( _SmallCascade, SmallCascadeSpeedValueMain600 ).a , clampResult488);
			...
			float lerpResult626 = lerp( ( tex2DNode213.a * 0.5 ) , tex2DNode213.a , clampResult807);
			...
			float clampResult850 = clamp( pow( ( temp_output_89_0 + ( 1.0 - _OutlinePower ) ) , ( 1.0 - _OutlineFalloffBorder ) ) , 0.0 , 1.0 );
			o.Emission = ( ( clampResult850 * half4(1,1,1,0) ) * i.vertexColor.a ).rgb;
			float lerpResult818 = lerp( _WaterSpecularFar , _WaterSpecularClose , pow( temp_output_94_0 , _WaterSpecularThreshold ));
			float lerpResult130 = lerp( lerpResult818 , _FoamSpecular , temp_output_114_0);
			...
			float clampResult849 = clamp( i.vertexColor.a , 0.0 , 1.0 );
			o.Alpha = ( lerpResult208 * clampResult849 );
		}
