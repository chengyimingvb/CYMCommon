		...
		protected virtual void DrawAddItemSection()
		{
			...
			using (new GUILayout.HorizontalScope())
			{
				...
				if (UIHelpers.IconButton(CSIcons.Plus, "Adds custom filter to the list.") || flag)
				{
					...
					else if (newItemKind == FilterKind.Extension && !newItemText.StartsWith("."))
					{
						window.ShowNotification(new GUIContent("Extension filter should start with dot (.dll)"));
					}
					else
					{
						if (CheckNewItem(ref newItemText))
						{
							if (CSEditorTools.TryAddNewItemToFilters(ref filters, FilterItem.Create(newItemText, newItemKind)))
							{
								SaveChanges();
								newItemText = "";
