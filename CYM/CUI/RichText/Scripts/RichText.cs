...
namespace CYM.UI
{
	...
	public class RichText : Text, ISerializationCallbackReceiver
	{
        ...
        [SerializeField]
        public bool EnableDynamicStr = false;
        [SerializeField]
        public bool EnableDynamicImage = false;
		...
		static class TagParser
		{
			...
			{
                //表情
                if (richText.EnableEmoji)
                {
                    text = Regex.Replace(text, BaseConstMgr.Regex_RichTextIcon, (m) =>
                    {
                ...
                //动态字符
                if (richText.EnableDynamicStr)
                {
                    text = Regex.Replace(text,BaseConstMgr.Regex_RichTextDyn, (m) =>
                    {
                        if (UIConfig.Ins.DynStrFuncs.ContainsKey(m.Groups[1].Value))
                        {
                            var temp = UIConfig.Ins.DynStrFuncs[m.Groups[1].Value].Invoke(null, null);
                            return string.Format("{0}",  temp);
                        }
                        else
                        {
                            return string.Format($"%{m.Groups[1]}%");
                        }
                    });

                    //text = text.Trim(BaseLanguageMgr.Space.ToCharArray());
                }
    ...
    class DynStr : System.Attribute
    {

    }
    [System.AttributeUsage(AttributeTargets.Method, AllowMultiple = true)]
    class DynIcon : System.Attribute
    {
