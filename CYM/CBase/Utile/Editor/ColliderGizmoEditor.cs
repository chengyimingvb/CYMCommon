...
namespace CYM
{
    ...
    public class ColliderGizmoEditor : Editor
    {
        ...
        {
            ColliderGizmo target = (ColliderGizmo)this.target;
            ...
            target.selectedPreset = (CGPresets)EditorGUILayout.EnumPopup(new GUIContent("Color Preset", "Presets color profiles that, when selected, will update your colors to preset values."), target.selectedPreset);
            ...
            {
                target.selectedPreset = CGPresets.Custom;
            }
            if (target.selectedPreset == CGPresets.Custom)
            {
